...
function navigateTo(pageKey) {
  ...
  } else if (pageKey === 'solar' || pageKey === 'solar-calculator') {
    if (window.SolarCalc) {
      window.SolarCalc.init();
    }
...
const routes = {
  ...
  solar: renderSolarPage,
  'solar-calculator': renderSolarPage,
...
function renderSolarCalculatorSection() {
  const t = window.t || ((k, d) => d || k);

  return `
    <!-- ===================================================================== -->
    <!-- COMPREHENSIVE SOLAR ENERGY CALCULATOR (Homes • Farms • Factories)   -->
    <!-- ===================================================================== -->
    <section class="section solar-calc-section" id="solar-calculator">
      <div class="container">
        ...
        <div class="solar-hub-header">
          <div>
            <div class="solar-badge">
              <i class="fa-solid fa-solar-panel"></i> <span>${t('solarHubBadge', 'المنظومة الوطنية للطاقة المتجددة 2026')}</span>
            </div>
            <h2 class="solar-main-title">${t('solarHubTitle', 'حاسبة الطاقة الشمسية الشاملة (منازل • مزارع • مصانع)')}</h2>
            <p class="solar-main-desc">${t('solarHubDesc', 'أداة هندسية ومالية ذكية لتقدير قدرة محطات الطاقة الشمسية، التكلفة الاستثمارية، والوفر المالي التراكمي لـ 1 و 5 و 10 سنوات، تدعم تقنيات الري النظيف وخفض الانبعاثات بالتعاون مع SGP/GEF/UNDP.')}</p>
          </div>

          <div style="background: rgba(255, 255, 255, 0.12); padding: 0.85rem 1.25rem; border-radius: 14px; border: 1px solid rgba(255, 255, 255, 0.2); text-align: center; min-width: 170px;">
            <div style="font-size: 0.8rem; color: #A7F3D0; font-weight: 700;">${t('solarTargetKw', 'القدرة المستهدفة')}</div>
            <div id="solar-res-kw" style="font-size: 1.8rem; font-weight: 900; color: #FDE68A; direction: ltr;">1.0 kW</div>
            <div style="font-size: 0.72rem; color: #E2ECE9;">${t('solarSmartEst', 'تقدير فوري ذكي')}</div>
          </div>
        </div>

        <!-- Dynamic Market Rates Bar (Editable) -->
        <div class="solar-market-bar">
          <div class="solar-market-header">
            <div style="font-weight: 900; font-size: 0.95rem; color: var(--color-primary, #064E3B); display: flex; align-items: center; gap: 0.5rem;">
              <i class="fa-solid fa-sliders text-gold"></i> <span>${t('solarMktTitle', 'متغيرات السوق والأسعار التقديرية اليوم (2026)')}</span>
            </div>
            <div class="solar-market-notice">
              <i class="fa-solid fa-circle-info"></i> <span>${t('solarMktNotice', 'تنبيه: يمكنك مراجعة وتعديل الأسعار أدناه لتطابق أسعار السوق وفاتورتك الفعلية:')}</span>
            </div>
          </div>

          <div class="solar-market-inputs-grid">
            <div class="solar-market-field">
              <label for="solar-mkt-elec">
                <span>${t('solarMktElecLabel', 'سعر ك.و.س كهرباء اليوم:')}</span>
              </label>
              <div class="input-wrap">
                <input type="number" id="solar-mkt-elec" step="0.1" min="0.5" max="20" value="${(window.SolarCalc && window.SolarCalc.elecPrice) || 2.8}">
                <span class="input-unit">ج.م / ك.و.س</span>
              </div>
            </div>

            <div class="solar-market-field">
              <label for="solar-mkt-kw-day">
                <span>${t('solarMktDayLabel', 'سعر كيلووات المحطة (نهاري):')}</span>
              </label>
              <div class="input-wrap">
                <input type="number" id="solar-mkt-kw-day" step="500" min="5000" max="150000" value="${(window.SolarCalc && window.SolarCalc.solarKwPriceDay) || 30000}">
                <span class="input-unit">ج.م / kWp</span>
              </div>
            </div>

            <div class="solar-market-field">
              <label for="solar-mkt-kw-hyb">
                <span>${t('solarMktHybLabel', 'سعر كيلووات المحطة (ببطاريات):')}</span>
              </label>
              <div class="input-wrap">
                <input type="number" id="solar-mkt-kw-hyb" step="1000" min="10000" max="250000" value="${(window.SolarCalc && window.SolarCalc.solarKwPriceHybrid) || 58000}">
                <span class="input-unit">ج.م / kWp</span>
              </div>
            </div>
          </div>
        ...
        <!-- Operating Mode Toggle (Day Only vs Day+Night Hybrid) -->
        <div class="solar-mode-container">
          <div class="solar-mode-title">
            <i class="fa-solid fa-bolt-lightning text-gold"></i> <span>${t('solarModeTitle', 'اختر نمط تشغيل الطاقة الشمسية المطلوب:')}</span>
          </div>
          <div class="solar-mode-toggle-grid">
            <div class="solar-mode-card ${(!window.SolarCalc || window.SolarCalc.systemMode === 'day') ? 'active' : ''}" data-mode="day" onclick="SolarCalc.setSystemMode('day')">
              <div class="solar-mode-icon">
                <i class="fa-solid fa-sun"></i>
              </div>
              <div>
                <h4>${t('solarModeDayTitle', '☀️ نهاري فقط (اقتصادي ومباشر On-Grid)')}</h4>
                <p>${t('solarModeDayDesc', 'تشغيل فوري للأجهزة وطلمبات الري أثناء سطوع الشمس بدون بطاريات. تكلفة استثمارية أقل واسترداد رأس مال سريع.')}</p>
              </div>
            </div>

            <div class="solar-mode-card ${(window.SolarCalc && window.SolarCalc.systemMode === 'hybrid') ? 'active' : ''}" data-mode="hybrid" onclick="SolarCalc.setSystemMode('hybrid')">
              <div class="solar-mode-icon">
                <i class="fa-solid fa-moon"></i>
              </div>
              <div>
                <h4>${t('solarModeHybTitle', '🌙 نهاري + ليلي (ببطاريات تخزين Hybrid)')}</h4>
                <p>${t('solarModeHybDesc', 'تشغيل مدار 24 ساعة مع منظومة بطاريات شمسية للتخزين الليلي، يضمن استقلالية كاملة عن انقطاع التيار.')}</p>
              </div>
            </div>
          </div>
        </div>

        <!-- Sector Tabs (Homes, Farms, Factories) -->
        <div class="solar-nav-tabs">
          <button type="button" class="solar-tab-btn ${(!window.SolarCalc || window.SolarCalc.activeTab === 'home') ? 'active' : ''}" data-tab="home" onclick="SolarCalc.switchTab('home')">
            <i class="fa-solid fa-house-chimney"></i> <span>${t('solarTabHome', '1. الطاقة الشمسية للمنازل')}</span>
          </button>
          <button type="button" class="solar-tab-btn ${(window.SolarCalc && window.SolarCalc.activeTab === 'farm') ? 'active' : ''}" data-tab="farm" onclick="SolarCalc.switchTab('farm')">
            <i class="fa-solid fa-wheat-awn"></i> <span>${t('solarTabFarm', '2. الطاقة الشمسية للمزارع والري')}</span>
          </button>
          <button type="button" class="solar-tab-btn ${(window.SolarCalc && window.SolarCalc.activeTab === 'factory') ? 'active' : ''}" data-tab="factory" onclick="SolarCalc.switchTab('factory')">
            <i class="fa-solid fa-industry"></i> <span>${t('solarTabFactory', '3. الطاقة الشمسية للمصانع')}</span>
          </button>
        </div>

        <!-- Active Calculator Layout (Inputs & Outputs) -->
        <div class="solar-calc-grid">
          
          <!-- LEFT / INPUTS COLUMN -->
          <div class="solar-inputs-card">
            
            <!-- PANEL 1: HOMES -->
            <div class="solar-tab-panel ${(!window.SolarCalc || window.SolarCalc.activeTab === 'home') ? 'active' : ''}" id="solar-panel-home">
              <h3 class="solar-card-title">
                <i class="fa-solid fa-list-check text-gold"></i> <span>${t('solarHomeCardTitle', 'قائمة الأجهزة المنزلية وتحديد العدد والتشغيل')}</span>
              </h3>
              <p class="solar-card-desc">${t('solarHomeCardDesc', 'حدد الأجهزة الموجودة في منزلك واضبط العدد وساعات التشغيل اليومية لتقدير إجمالي الحمل وحجم المحطة المطلوب:')}</p>
              
              <!-- Appliances dynamic grid (populated by JS) -->
              <div class="appliances-grid" id="solar-appliances-grid"></div>
            </div>

            <!-- PANEL 2: FARMS -->
            <div class="solar-tab-panel ${(window.SolarCalc && window.SolarCalc.activeTab === 'farm') ? 'active' : ''}" id="solar-panel-farm">
              <h3 class="solar-card-title">
                <i class="fa-solid fa-tractor text-gold"></i> <span>${t('solarFarmCardTitle', 'مواصفات المزرعة ومنظومة ضخ المياه')}</span>
              </h3>
              <p class="solar-card-desc">${t('solarFarmCardDesc', 'اختر نوع النشاط الزراعي وطريقة الري أو أدخل عدد الكيلووات المطلوبة مباشرة:')}</p>

              <div class="solar-input-group">
                <label class="solar-label" for="solar-farm-type">${t('solarFarmTypeLabel', 'نوع المزرعة / النشاط:')}</label>
                <select id="solar-farm-type" class="solar-select">
                  <option value="green" selected>${t('solarFarmTypeGreen', 'مزرعة نباتية ومحاصيل حقلية')}</option>
                  <option value="animal">${t('solarFarmTypeAnimal', 'مزرعة إنتاج حيواني ودواجن')}</option>
                  <option value="fish">${t('solarFarmTypeFish', 'مزرعة استزراع سمكي (بدالات وهوايات)')}</option>
                </select>
              </div>

              <div class="solar-input-group">
                <label class="solar-label" for="solar-farm-irrig">${t('solarFarmIrrigLabel', 'طريقة الري ومصدر المياه:')}</label>
                <select id="solar-farm-irrig" class="solar-select">
                  <option value="drip" selected>${t('solarIrrigDrip', 'ري بالتنقيط (ضغط منخفض - موفر للطاقة)')}</option>
                  <option value="sprinkler">${t('solarIrrigSprinkler', 'ري بالرش / محوري (Pivot)')}</option>
                  <option value="flood">${t('solarIrrigFlood', 'ري بالغمر السطحي (قنوات مائية)')}</option>
                  <option value="well">${t('solarIrrigWell', 'طلمبة غاطسة من بئر عميق (أكثر من 50 متر)')}</option>
                </select>
              </div>

              <div class="solar-input-group">
                <label class="solar-label">${t('solarFarmMethodLabel', 'طريقة تحديد قدرة المنظومة:')}</label>
                <div class="solar-pills-row">
                  <button type="button" class="solar-pill-choice active solar-farm-mode-btn" data-mode="hp" onclick="SolarCalc.setFarmInputMode('hp')">${t('solarFarmModeHp', 'عبر قدرة طلمبة الري (بالحصان HP)')}</button>
                  <button type="button" class="solar-pill-choice solar-farm-mode-btn" data-mode="kw" onclick="SolarCalc.setFarmInputMode('kw')">${t('solarFarmModeKw', 'إدخال عدد الكيلووات مباشرة (kW)')}</button>
                </div>
              </div>

              <div id="solar-farm-hp-wrap" class="solar-input-group">
                <label class="solar-label">
                  <span>${t('solarFarmHpLabel', 'قدرة طلمبة الري (حصان):')}</span>
                  <strong id="solar-farm-hp-val" style="color: #059669; font-size: 1.1rem;">10</strong> HP
                </label>
                <input type="range" id="solar-farm-hp" min="1" max="100" step="1" value="10" style="width: 100%; accent-color: #10B981;">
              </div>

              <div id="solar-farm-kw-wrap" class="solar-input-group" style="display: none;">
                <label class="solar-label" for="solar-farm-kw">${t('solarFactKwLabel', 'عدد الكيلووات المطلوبة (kW): *')}</label>
                <input type="number" id="solar-farm-kw" class="solar-input" min="1" max="1000" step="1" value="15" placeholder="15">
              </div>
            </div>

            <!-- PANEL 3: FACTORIES -->
            <div class="solar-tab-panel ${(window.SolarCalc && window.SolarCalc.activeTab === 'factory') ? 'active' : ''}" id="solar-panel-factory">
              <h3 class="solar-card-title">
                <i class="fa-solid fa-city text-gold"></i> <span>${t('solarFactCardTitle', 'حساب الطاقة الشمسية للمصانع والمنشآت')}</span>
              </h3>
              <p class="solar-card-desc">${t('solarFactCardDesc', 'اكتفِ بوضع عدد الكيلووات المطلوبة أو الحمل الأقصى للمصنع لحساب التكلفة والوفر المالي فوراً:')}</p>

              <div class="solar-input-group">
                <label class="solar-label" for="solar-fact-kw">${t('solarFactKwLabel', 'عدد الكيلووات المطلوبة للمصنع (kW): *')}</label>
                <input type="number" id="solar-fact-kw" class="solar-input" min="5" max="2000" step="5" value="50" placeholder="50">
              </div>

              <div class="solar-input-group">
                <label class="solar-label" for="solar-fact-bill">${t('solarFactBillLabel', 'متوسط فاتورة الكهرباء الشهرية للمصنع (ج.م) - اختياري:')}</label>
                <input type="number" id="solar-fact-bill" class="solar-input" min="1000" step="1000" value="50000" placeholder="50000">
              </div>
            </div>
          ...
          <div class="solar-results-card">
            <div class="solar-res-header">
              <div>
                <h3 style="font-size: 1.15rem; font-weight: 900; color: #064E3B; margin: 0;">${t('solarResTitle', 'مؤشرات المحطة والوفر المالي')}</h3>
                <span style="font-size: 0.78rem; color: #64748B;">${t('solarResSub', 'نتائج تقديرية فورية متفاعلة')}</span>
              </div>
              <span class="solar-res-badge">2026 Updated</span>
            </div>

            <!-- Main Capex Highlight -->
            <div class="solar-kpi-main">
              <div class="solar-kpi-label">${t('solarResCostLabel', 'التكلفة التقديرية لإنشاء المحطة:')}</div>
              <div class="solar-kpi-value" id="solar-res-cost">0 ج.م</div>
              <div class="solar-kpi-sub">${t('solarResCostSub', 'تشمل الألواح والإنفرتر والهياكل والتركيب والتشغيل')}</div>
            </div>

            <!-- Cumulative Savings Timeline (1, 5, 10 Years) -->
            <div class="solar-timeline-title">
              <i class="fa-solid fa-coins text-gold"></i> <span>${t('solarSavingsTitle', 'الوفر المالي التراكمي المتوقع:')}</span>
            </div>
            <div class="solar-timeline-grid">
              <div class="solar-time-card">
                <div class="solar-time-period">${t('solarYr1', 'خلال سنة واحدة')}</div>
                <div class="solar-time-amount" id="solar-res-yr1">0 ج.م</div>
              </div>
              <div class="solar-time-card">
                <div class="solar-time-period">${t('solarYr5', 'خلال 5 سنوات')}</div>
                <div class="solar-time-amount" id="solar-res-yr5">0 ج.م</div>
              </div>
              <div class="solar-time-card">
                <div class="solar-time-period">${t('solarYr10', 'خلال 10 سنوات')}</div>
                <div class="solar-time-amount" id="solar-res-yr10">0 ج.م</div>
              </div>
            </div>

            <!-- Technical & Environmental Sub-KPIs -->
            <div class="solar-sub-kpis">
              <div class="solar-mini-kpi">
                <div class="solar-mini-label">${t('solarResRoiLabel', 'فترة استرداد رأس المال:')}</div>
                <div class="solar-mini-val text-gold" id="solar-res-roi">--</div>
              </div>
              <div class="solar-mini-kpi">
                <div class="solar-mini-label">${t('solarResCo2Label', 'خفض الكربون سنوياً:')}</div>
                <div class="solar-mini-val text-emerald" id="solar-res-co2">--</div>
              </div>
              <div class="solar-mini-kpi">
                <div class="solar-mini-label">${t('solarResPanelsLabel', 'عدد الألواح التقريبي:')}</div>
                <div class="solar-mini-val" id="solar-res-panels">--</div>
              </div>
              <div class="solar-mini-kpi">
                <div class="solar-mini-label">${t('solarResAreaLabel', 'المساحة المطلوبة:')}</div>
                <div class="solar-mini-val" id="solar-res-area">--</div>
              </div>
            </div>

            <!-- CTA Button -->
            <a href="#" id="solar-btn-whatsapp" target="_blank" rel="noopener" class="solar-btn-whatsapp">
              <i class="fa-brands fa-whatsapp" style="font-size: 1.25rem;"></i>
              <span>${t('solarCtaWa', 'اطلب دراسة فنية وعرض سعر رسمي')}</span>
            </a>
          </div>

        </div>

        <!-- Prominent Technical & Legal Disclaimer -->
        <div class="solar-disclaimer-box">
          <div class="solar-disclaimer-icon">
            <i class="fa-solid fa-triangle-exclamation"></i>
          </div>
          <div class="solar-disclaimer-content">
            <h4>${t('solarDisclaimerTitle', 'تنويه وإخلاء مسؤولية فني وقانوني هام:')}</h4>
            <p>
              ${t('solarDisclaimerText', 'جميع الأرقام والتكاليف والوفورات المالية الناتجة عن هذه الحاسبة هي أرقام استرشادية وتقديرية مبنية على متوسطات السوق الحالية لعام 2026. قد تختلف هذه الأرقام على أرض الواقع الفعلي وفقاً لعدة متغيرات، منها: الموقع الجغرافي للمشروع، زاوية ميل وإشعاع الشمس، عمق المياه الجوفية ونوع التربة، نوع وماركة الألواح والإنفرتر ومحولات الطاقة، كفاءة وتذبذب أسعار الصرف ومكونات الاستيراد، والتعريفة الرسمية المعتمدة لشركات توزيع الكهرباء. يرجى التواصل لطلب معاينة هندسية ميدانية ودراسة جدوى فنية متخصصة قبل التعاقد.')}
            </p>
          </div>
        </div>
...
function renderSolarPage() {
  ...
  return `
    <header class="home-hero-section" style="padding: 3.5rem 0 3rem;">
      <div class="container">
        <h1 class="hero-main-title" style="font-size: 2.35rem;">
          <i class="fa-solid fa-solar-panel text-gold"></i> ${t('solarHubTitle', 'حاسبة الطاقة الشمسية الشاملة (منازل • مزارع • مصانع)')}
        </h1>
        <p class="hero-lead-text">
          ${t('solarHubDesc', 'أداة هندسية ومالية ذكية لتقدير قدرة محطات الطاقة الشمسية، التكلفة الاستثمارية، والوفر المالي التراكمي لـ 1 و 5 و 10 سنوات، تدعم تقنيات الري النظيف وخفض الانبعاثات بالتعاون مع SGP/GEF/UNDP.')}
        </p>
      </div>
    </header>
